...
\begin{enumerate}[(a)]
	...
	\lstinputlisting[language=R, firstline=50, lastline=53]{PS5_Answer.R}  
	%\newpage
...
\newpage
	\item Check the normality assumption with a Q-Q plot of the studentized residuals. \vspace{.25cm}
		\lstinputlisting[language=R, firstline=57, lastline=57]{PS5_Answer.R}  
		%\newpage
...
\item Check for large leverage points by plotting the $h$ values. \vspace{.25cm}
	\lstinputlisting[language=R, firstline=61, lastline=65]{PS5_Answer.R}  
	%\newpage
	
	\vspace{.5cm}
	\begin{figure}[h!]\centering
	\caption{\footnotesize Hatvalues}\vspace{-1cm}
	\label{fig:c}
	\includegraphics[width=.85\textwidth]{c.pdf}\\
	\end{figure}
	The graph is on page 4...
\item Check for outliers by running an \texttt{outlierTest}. \vspace{.25cm}
		\lstinputlisting[language=R, firstline=69, lastline=69]{PS5_Answer.R} 
		\begin{footnotesize}	
			\begin{verbatim}
...
\item Check for influential points by creating a "Bubble plot" with the hat-values and studentized residuals.\\
	\vspace{.25cm}
	\lstinputlisting[language=R, firstline=72, lastline=73]{PS5_Answer.R}  
